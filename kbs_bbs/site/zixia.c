...
int uleveltochar( char* buf, struct userec *lookupuser )
{
	...
	//
    	...
    	//else if( lvl & PERM_SYSOP ) strcpy(buf,"帮主");
      	else if( lvl & PERM_COLLECTIVE && lvl & PERM_CHATCLOAK ) strcpy(buf,"晶晶姑娘");
      	else if( lvl & PERM_COLLECTIVE ) strcpy(buf,"观音美眉");
...
int old_compute_user_value(const struct userec *urec)
{
    ...
    * */
    ...
    if( urec->userlevel & PERM_COLLECTIVE )
